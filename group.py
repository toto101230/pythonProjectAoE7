    ...
    def update(self):
        ...
        if not pygame.mouse.get_pressed()[0]:  #clic gauche pas appuyé
            if self.init_pos is not None:
                self.rec_coord = (self.init_pos[0], self.init_pos[1], pygame.mouse.get_pos()[0] - self.init_pos[0], pygame.mouse.get_pos()[1] - self.init_pos[1])  #coordonées sur l'écran du rectangle tracé
            self.init_pos = None
